...
//            msg->header.frame_id != _ci->getBaseLink(ee_name)) // and is not consistent with CI..
//    {
...
//    _ci->setPoseReference(ee_name, T);
...
//    _ci->setForceReference(ee_name, f);
//}
...
//    for(std::string ee_name : _ci->getTaskList())
//    {
//        if(_ci->getTaskInterface(ee_name) != TaskInterface::Interaction)
//        {
//            continue;
//        }
...
//    {
//        _model->getOrientation(_ci->getBaseLink(ee_name), b_R_f);
//        b_R_f.transposeInPlace();
//    }
//    else if(msg->header.frame_id != "")
//    {

//        if(!_model->getOrientation(msg->header.frame_id, _ci->getBaseLink(ee_name), b_R_f))
//        {
...
//    _ci->setVelocityReference(ee_name, vel);
...
//    for(std::string ee_name : _ci->getTaskList())
//    {
//        _action_managers.emplace_back(_nh, ee_name, _ci);
//    }
//}
...
//    if(!_ci->getReferencePosture(_posture_ref))
//    {
//        return;
//    }
...
//        _ci->getVelocityLimits(task, max_vel_lin, max_vel_ang);
//        _ci->getAccelerationLimits(task, max_acc_lin, max_acc_ang);
...
//        _ci->setVelocityLimits(task, max_vel_lin, max_vel_ang);
//        _ci->setAccelerationLimits(task, max_acc_lin, max_acc_ang);
//    }
...
//    res.base_link = _ci->getBaseLink(ee_name);
//    res.control_mode  =  CartesianInterface::ControlTypeAsString(_ci->getControlMode(ee_name));
//    res.task_state  =  CartesianInterface::StateAsString(_ci->getTaskState(ee_name));
//    res.distal_link = ee_name;
//    res.task_interface = CartesianInterface::TaskInterfaceAsString(_ci->getTaskInterface(ee_name));
...
//    if(new_base_link != "" && _ci->setBaseLink(ee_name, new_base_link))
//    {
...
//    if(req.control_mode != "" && _ci->setControlMode(ee_name, CartesianInterface::ControlTypeFromString(req.control_mode)))
//    {
...
//    _ci->setReferencePosture(posture_ref);
//}
...
//    if(_ci->setReferencePosture(posture_ref))
//    {
...
//    if(!_ci->getDesiredInteraction(ee_name, f, k, d))
//    {
//        return false;
//    }
...
//    {
//        _ci->setDesiredStiffness(ee_name, k);
//        _ci->setDesiredDamping(ee_name, d);
//    }
//}
...
//        Eigen::Affine3d base_T_ee;
//         _ci->getCurrentPose(_ee_name, base_T_ee);
...
//        if(!_ci->setWayPoints(_ee_name, waypoints))
//        {
...
//    if(_ci->getTaskState(_ee_name) == State::Reaching)
//    {
//        Logger::info(Logger::Severity::HIGH,
...
//        _ci->getCurrentPose(_ee_name, base_T_ee);
//        _ci->getPoseReference(_ee_name, base_T_ref);
...
//    // trajectory ended
//    if(_ci->getTaskState(_ee_name) == State::Online)
//    {
...
//        feedback.current_reference.header.frame_id = _ci->getBaseLink(_ee_name);
//        _ci->getPoseReference(_ee_name, base_T_ref);
//        tf::poseEigenToMsg(base_T_ref, feedback.current_reference.pose);

//        Eigen::Affine3d base_T_ee;
//        feedback.current_pose.header.stamp = ros::Time::now();
//        feedback.current_pose.header.frame_id = _ci->getBaseLink(_ee_name);
//        _ci->getCurrentPose(_ee_name, base_T_ee);
//        tf::poseEigenToMsg(base_T_ee, feedback.current_pose.pose);

//        feedback.current_segment_id = _ci->getCurrentSegmentId(_ee_name);
