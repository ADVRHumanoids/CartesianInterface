...
Aeronav::Ptr joystick;

void constructAeronav(ros::NodeHandle nh, Aeronav::Ptr& joystick)
{
    /* Get task list from cartesian server */
    ros::ServiceClient task_list_client = nh.serviceClient<cartesian_interface::GetTaskListRequest, 
                                                           cartesian_interface::GetTaskListResponse>("get_task_list");
    ...
    joystick = std::make_shared<XBot::Cartesian::Aeronav>(distal_links, base_links, tf_prefix);
...
int main(int argc, char **argv){
    ...
    /* Init ROS node */
    ros::init(argc, argv, "aeronav_spawner");
